package com.amarkhel.day4.repository.params

import java.util.Date

import com.amarkhel.day4.repository.{Account, AccountRepository, AccountRepositoryInMemory, Balance}
...
object App1 extends App {
  ...
  def op(no: String):Try[Balance] = for {
    a <- credit(no, BigDecimal(100), r)
    c <- credit(no, BigDecimal(300), r)
    d <- debit(no, BigDecimal(160), r)
    b <- balance(no, r)
  } yield b
  val balance2: Try[Balance] = op("1")
  balance2 match{
    case Failure(e) => println(e.getMessage)
    case Success(b) => println(b.amount)
  }
  r.store(Account("1", "Andrey"))
  val balance3: Try[Balance] = op("1")
  balance3 match{
    case Failure(e) => println(e.getMessage)
    case Success(b) => println(b.amount)
  }
